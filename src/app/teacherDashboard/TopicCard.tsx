...
export default function TopicCard({
  ...
  const displayPercentage = isNaN(performancePercentage) ? 'NA' : `${performancePercentage}`;
  ...
  >
    ...
    <div className="flex items-center">
      <span className="text-sm">{(totalMarksAchieved/9).toFixed(2)}</span>
      <span className="ml-1 text-sm">/</span>
      <span className="ml-1 text-sm">{(totalMarks/9)}</span>
    </div>
