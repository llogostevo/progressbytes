...
export default function AddUnitForm({ slug }: { slug: string; }) {
    ...
    async function handleFormSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const { data, error } = await supabase
            .from('unittable')
            .insert([
                { courseid: `${slug}`, unittitle: `${formData.unittitle}`, unitnumber: `${formData.unitnumber}`, },
            ])
            .select()
    ...
    return (
        ...
        >
            ...
            <div className="mb-4">
                <label htmlFor="unittitle" className="block text-sm font-medium text-gray-700">Unit Name:</label>
                <input
                    type="text"
                    id="unittitle"
                    name="unittitle"
