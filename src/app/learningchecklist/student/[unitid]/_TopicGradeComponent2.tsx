...
const NewTopicGrade = ({ studentId, unitId, assessmentType, startDate, endDate }: Props) => {
    ...
    const fetchHistoricalPerformance = async () => {
        ...
        // Set State to be empty prior to new fetch of data
    ...
    const fetchUnittableData = async () => {
        ...
        console.log("pef data", historicalPerformanceData)
    ...
    // USE EFFECT FOR CALLING THE FETCH DATA FROM HISTORICAL PERFORMANCE

    // useEffect(() => {
    //     fetchHistoricalPerformance();
    //     // setFilterKey(prev => prev + 1);
    // }, [unitId, studentId, assessmentType, startDate, endDate, supabase]);

    useEffect(() => {
        const fetchData = async () => {
            await fetchHistoricalPerformance(); // This fetches and sets historical data
            fetchUnittableData(); // This is called right after the above line completes
        };

        fetchData();
    }, [unitId, studentId, assessmentType, startDate, endDate, supabase]);

    // Removed the useEffect for fetchUnittableData as it's now included in the above useEffect
    ...
    // useEffect(() => {
    //     // Sort the historical performance data
    //     const sortedHistoricalPerformance = [...historicalPerformanceData].sort((a, b) => {
    //         return a.topic_number.localeCompare(b.topic_number, undefined, { numeric: true });
    //     });
    //     setHistoricalPerformanceData(sortedHistoricalPerformance);
    // }, []);
