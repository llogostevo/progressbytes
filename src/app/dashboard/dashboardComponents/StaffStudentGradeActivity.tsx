...
export default function StaffStudentGradeActivity({ studentId }: { studentId: number }) {
    // ################ REACT STATES ################ 
                ...
                <div className="ml-10 mt-4 flex flex-col mb-1 md:mb-2 lg:mb-4">
                    {units.map((unit, idx) => (
                        <label key={idx} className="flex items-center mb-1">
                            ...
                            />
                            <Link className="p-2 text-xs md:text-md lg:text-lg" href={`../teacher/${studentId}/studentplc?unitid=${unit.unitid}&studentid=${studentId}`}>{unit.unitnumber} - {unit.unittitle}</Link>
                ...
                <div className="mx-5">
                    ...
                    <div className="flex flex-col space-y-2">
                        ...
                        {top5Subtopics.map((subtopic, index) => (
                            <div className="text-[13px] text-gray-500 py-3" key={index}>
                                <Link href={`../teacher/${studentId}/studentplc?unitid=${subtopic.unitId}&studentid=${studentId}#${subtopic.topicName}`}>{subtopic.subtopic} - <span className=' text-green-600'>{subtopic.percentage.toFixed(2)}% ({subtopic.totalMarks} / {subtopic.totalQuestionMarks})</span></Link>
                    ...
                    <div className="flex flex-col space-y-2">
                        ...
                        {bottom5Subtopics.map((subtopic, index) => (
                            <div className="text-[13px] text-gray-500 py-3" key={index}>
                                <Link href={`../teacher/${studentId}/studentplc?unitid=${subtopic.unitId}&studentid=${studentId}#${subtopic.topicName}`}>{subtopic.subtopic} - <span className=' text-red-700'>{subtopic.percentage.toFixed(2)}% ({subtopic.totalMarks} / {subtopic.totalQuestionMarks})</span></Link>
                            </div>
                        ))}
                    </div>
                </div>
